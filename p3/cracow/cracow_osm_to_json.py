        ...
        for tag in element.iter('tag'):
            ...
            if problemchars.search(k) is not None:
                ...
                if segments[0] == "addr":
                    if len(segments) != 2:
                        continue
                    # use data/knowledge from audit scripts to fix postal codes and nodes' street names.
                    if k == 'addr:postcode':
                        v = audit_postal_codes.fixed_postal_code(v)[1]
                        # if type(v) == unicode:
                        #     encoded_v = v.encode('utf-8')
                        #     fixed = audit_postal_codes.fixed_postal_code(encoded_v)[1]
                        #     v = fixed.decode('utf-8')
                        # else:
                        #     v = audit_postal_codes.fixed_postal_code(v)[1]
                    if k == 'addr:street' and v in audit_street_names_exceptions.STREETNAME_FIXINGS:
                        v = audit_street_names_exceptions.STREETNAME_FIXINGS[v]
...
def process():
    ...
    data = process_map('krakow_poland.osm', False)
    # pprint.pprint(data)
