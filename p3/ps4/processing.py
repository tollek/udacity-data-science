...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            # if 'name' is NULL or contains non-alphanumeric characters, replace it with label
            if p['name'] == 'NULL' or not p['name'].isalnum():
                p['name'] = p['label']
            # Replace or 'NULL's with Nones
            for k, v in p.iteritems():
                if v == 'NULL':
                    p[k] = None
            # If there is value in 'synonyms', built an array out of it.
            if p['synonym'] is not None:
                p['synonym'] = [x.strip() for x in p['synonym'].split('|')]
            # strip leading and ending whitespace from all fields, if there is any
            for k, v in p.iteritems():
